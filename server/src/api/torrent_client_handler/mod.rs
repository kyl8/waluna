...
// Handler: GET /list_files?id=... - Lista arquivos
pub async fn list_files_handler(
    ...
    let client = match get_client().await {
        ...
        Err(e) => {
            return Response::builder()
                ...
                .body(format!("Failed to initialize client: {}", e).into())
                .unwrap();
        }
    };

    let download_id = match params.get("id") {
        Some(id) => id.clone(),
        None => {
            return Response::builder()
                .status(StatusCode::BAD_REQUEST)
    ...
    // Busca o download pelo ID
    let downloads = client.list_downloads().await;
    let download_info = match downloads.iter().find(|d| d.download_id == download_id) {
    ...
    let payload = json!({
        ...
        "progress": format!("{:.1}%", download_info.progress),
